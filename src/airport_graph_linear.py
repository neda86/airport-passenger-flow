...
LAYOUT_NAME = "linear"
# Landside nodes that receive the "departure pressure" schedule covariate.
SCHED_DEP_NODES = ["Arrival", "Check-in", "Security"]
# Hub node whose sched_board aggregates its gates' boarding.
GATE_HUB = {g: "Boarding" for g in GATES}


def node_flow_events(df):
    """One row per (node, flow-type) event, timestamped — linear layout."""
    import pandas as pd
    ...
    recs = []
    def add(node, start_col, end_col, sub=None):
        sub = (df if sub is None else sub)[[start_col, end_col]].dropna()
